...
| POST | `/waitlist` | Body: `facility_id`, `reservation_date`, `time_slot`, optional `purpose`. Join the waitlist for a facility/date/time — typically called after a `409 conflict` from `POST /reservations`, using the `facility_id`/`reservation_date`/`time_slot` echoed back in that error's payload. Returns `409 already_waitlisted` if already on the list for that exact slot. |
| GET | `/waitlist` | Resident's own entries: `waiting` / `offered` / `claimed` / `expired` / `cancelled`. An `offered` entry includes `offer_expires_at` — claim it by submitting a normal `POST /reservations` for that facility/date/time before it expires. |
| POST | `/waitlist/{id}/cancel` | Leave the waitlist. `404` if not found, not yours, or already resolved. |
