...
- As a resident, I see AI conflict warnings and alternative time slots (including PH holiday/Barangay Culiat event risk) when a booking conflicts so I can adjust my request. **(OPTIMIZED: ~60% faster conflict detection, ~70% fewer API calls)**
- As a resident, I see AI facility recommendations (including distance) based on my purpose/location so I can pick suitable venues. **(OPTIMIZED: Timeout protection, smart fetching, debounced 1000ms)**
...
- As an admin/staff, I can paginate through pending requests on the dashboard (3 items per page) so I can manage large volumes of requests efficiently without cluttering the view.
