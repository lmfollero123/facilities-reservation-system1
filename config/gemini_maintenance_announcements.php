...
PROMPT;
    ...
    $payloadBase = [
        ...
        'contents' => [
            ['role' => 'user', 'parts' => [['text' => "Create the maintenance announcement using these facts:\n\n" . $facts]]],
        ],
        'generationConfig' => [
            'temperature' => 0.45,
            'maxOutputTokens' => 600,
    ...
    foreach ($models as $model) {
        ...
        error_log("Gemini maintenance announcement model {$model} failed: HTTP {$httpCode}");
    }
    ...
    if (!is_array($parsed)) {
        if (preg_match('/\{[\s\S]*"title"[\s\S]*"message"[\s\S]*\}/', $text, $m)) {
